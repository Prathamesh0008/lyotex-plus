...
export default function ServicesPage() {
  return <>
    <PageIntro title="Services" description="Biopharmaceutical development services, from product selection and process development to technical transfer and regulatory support." />
    <nav className={`site-container ${styles.jump}`} aria-label="Service sections">{services.map(s => <a key={s.id} href={`#${s.id}`}>{s.title}</a>)}</nav>
    <div>{services.map((service) => {
      ...
      return <section key={service.id} id={service.id} className={styles.detail} aria-labelledby={`${service.id}-heading`}>
        <div className={`site-container ${styles.row}`}>
          <div className={styles.copy}><h2 id={`${service.id}-heading`}>{service.title}</h2><p>{detail.description}</p><h3>Key capabilities</h3><ul>{detail.capabilities.map(c => <li key={c}>{c}</li>)}</ul></div>
